...
class DuellingQNetworkAgent:
    ...
    def _build_model(self):
        ...
        # Common learning layer
        net = Dense(self.hidden_size)(inputs)

        # state value stream
        value_hidden = Dense(self.hidden_size2)(net)
        value_output = Dense(1)(value_hidden)
        value_output = Lambda(lambda value: tf.tile(value, [1, self.num_actions]))(value_output)

        # action advantage stream
        action_hidden = Dense(self.hidden_size2)(net)
        action_output = Dense(self.num_actions)(action_hidden)
        action_output = Lambda(lambda advt: advt - tf.reduce_mean(
            advt, axis=-1, keepdims=True))(action_output)

        #combine state and action to get the final Q value
        final = Add()([value_output, action_output])
