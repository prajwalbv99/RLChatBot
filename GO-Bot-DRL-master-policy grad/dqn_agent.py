...
class DQNAgent:
    ...
    def __init__(self, state_size, constants):
        ...
        self.states_lis, self.actions_lis, self.rewards_lis = [], [], []
        self.next_state_lis, self.done_lis = [], []
    ...
    def _build_model(self):
        ...
        model.add(Dense(self.hidden_size, input_dim=self.state_size, activation='relu',kernel_initializer='glorot_uniform'))
        model.add(Dense(self.hidden_size, activation='relu',kernel_initializer='glorot_uniform'))
        model.add(Dense(self.num_actions, activation='softmax',kernel_initializer='glorot_uniform'))
    ...
    def _dqn_predict_one(self, state, target=False):
        ...
        Returns:
            numpy.array
        """
        policy = self.policy_model.predict(state.reshape(1,self.state_size), batch_size=1).flatten()
    ...
    def append_sample(self, state, action, reward, next_state, done):
        self.states_lis.append(state)
        self.rewards_lis.append(reward)
        self.actions_lis.append(action)
        self.next_state_lis.append(next_state)
        self.done_lis.append(done)
    ...
    def train(self):
        ...
        """
        episode_length = len(self.states_lis)
        print(episode_length)
        discounted_rewards = self.discount_rewards(self.rewards_lis)
        ...
        for i in range(episode_length):
            update_inputs[i] = self.states_lis[i]
            advantages[i][self.actions_lis[i]] = discounted_rewards[i]

        self.policy_model.fit(update_inputs, advantages, epochs=1, verbose=0)
        self.states_lis, self.actions_lis, self.rewards_lis,self.next_state_lis,self.done_lis = [], [], [],[],[]
    ...
    def save_weights(self):
        ...
        if not self.save_weights_file_path:
            return
        save_file_path = re.sub(r'\.h5', r'_policy_model.h5', self.save_weights_file_path)
        self.policy_model.save_weights(save_file_path)
    ...
    def _load_weights(self):
        ...
        if not self.load_weights_file_path:
            return
        load_file_path = re.sub(r'\.h5', r'_policy_model.h5', self.load_weights_file_path)
        self.policy_model.load_weights(load_file_path)
