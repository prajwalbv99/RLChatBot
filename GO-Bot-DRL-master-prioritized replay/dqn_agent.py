...
class DQNAgent:
    ...
    def add_experience(self, state, action, reward, next_state, done):
        ...
        self.memory[self.memory_index] = [state, action, reward, next_state, done,0]
    ...
    def train(self):
        ...
        # Calc. num of batches to run                           state_zie = 224   num_actions = 39
        #print(self.num_actions)
        num_batches = len(self.memory) // self.batch_size
        for b in range(num_batches):
            #batch = random.sample(self.memory, self.batch_size)
            #print(self.state_size)
            probs_list = [item[5] for item in self.memory]
            batch_indices  = np.random.choice(np.arange(len(self.memory)),self.batch_size,probs_list)
            ...
            for i, (s, a, r, s_, d,td_err) in enumerate(batch):
                t = beh_state_preds[i]
                if not self.vanilla:
                    t[a] = r + self.gamma * tar_next_state_preds[i][np.argmax(beh_next_states_preds[i])] * (not d)
                    self.memory[batch_indices[i]][5] = abs(r + (self.gamma * tar_next_state_preds[i][np.argmax(beh_next_states_preds[i])]*(not d)-beh_state_preds[i][a]))
                else:
                    #print("hello")
                    t[a] = r + self.gamma * np.amax(tar_next_state_preds[i]) * (not d)
                    self.memory[batch_indices[i]][5] = abs(r + (self.gamma * np.amax(tar_next_state_preds[i])*(not d)-beh_state_preds[i][int(a)]))
                    #print(self.memory[batch_indices[i]])
                inputs[i] = s
                targets[i] = t
