...
def pg_loss(rewards):
    def loss(labels, predicted_output):
        ...
        ################################
        #logloss = -(labels * K.log(predicted_output) + (1 - labels) * K.log(1 - predicted_output))
        #avg_loss = K.mean(logloss, axis=-1)*rewards
        avg_loss = K.sparse_categorical_crossentropy(labels,predicted_output)*rewards
        return avg_loss
    return loss
...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        d1 = Dense(self.hidden_size, activation='relu')(states)
        #d2 = Dense(64, activation='relu')(d1)
        #d3 = Dense(64, activation='relu')(d1)
        do = Dense(self.num_actions)(d1)
        out = Softmax()(do)
    ...
    def _dqn_predict_one(self, state, target=False):
        ...
        Returns:
            numpy.array
        """
        return self.policy_network.predict([[state], np.zeros((1, 1))])[0]
    ...
    def _dqn_predict(self, states, target=False):
        ...
        return self.policy_network.predict([[states], np.zeros((1, 1))])[0]
    ...
    def train(self,episodes):
        ...
        rewards = []
        train_labels = []
        #print("Action space"+str(self.num_actions))
        for state,action,reward in episodes[::-1]:
                #act_list = [0]*self.num_actions
                G_est = G_est*self.gamma+reward
                #print(action)
                #act_list[action] = 1
                train_labels.append(action)
                train_data.append(state)
                rewards.append(G_est)
        train_labels = np.array(train_labels)
        #print("train labels shape"+str(train_labels.shape))
        self.policy_network.fit([train_data,rewards],train_labels)
    ...
    def save_weights(self):
        ...
        policy_model_savepath = re.sub(r'\.h5', r'_beh.h5', self.save_weights_file_path)
        self.policy_network.save_weights(policy_model_savepath)
    ...
    def _load_weights(self):
        ...
        policy_model_loadpath = re.sub(r'\.h5', r'_beh.h5', self.load_weights_file_path)
        self.policy_netowrk.load_weights(policy_model_loadpath)
