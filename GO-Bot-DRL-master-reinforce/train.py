...
def train_run():
    ...
    while episode < NUM_EP_TRAIN:
        ...
        print("Episode num:"+str(episode))
        state = state_tracker.get_state()
        while not done:
            next_state, reward, done, success, st_ac_tuple = run_round(state)
            period_reward_total += reward
            sars_episodes.append([state,st_ac_tuple[1],reward])
            state = next_state
        ...
        if episode % TRAIN_FREQ == 0:
            ...
            for state,action,reward in episode[::-1]:
                G_est = G_est*GAMMA+reward
                train_labels.append(action.tolist())
                train_data.append(state.tolist())
